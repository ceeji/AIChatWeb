...
export function buildToolSystemPrompt(tools: ToolDefinition[]): string {
  ...
  const planningSection = hasTodoWrite
    ? `
**任务规划要求（必须遵守）：**
- 接到复杂任务后，**第一步必须调用 todowrite 工具**列出完整执行计划，todowrite 必须是多个工具调用中的第一个；所有步骤初始状态为 pending
...
2. 在输出 <tool_call> 之前，你可以先用自然语言简要说明你要做什么（例如"让我来搜索一下..."），如果你要调用多个工具，所有说明必须都写在开头，不能夹杂在 tool_call 之间或之后。
3. 一旦开始输出 <tool_call>，之后禁止再输出其他文本，等待工具执行结果
